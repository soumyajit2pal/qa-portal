...
const itemName = (value: string) =>
  value.split("/").filter(Boolean).pop() || "Document Portal";
...
function FolderTree({
  ...
  const tree = useMemo(
    () =>
      folders.reduce<Record<string, Folder[]>>((out, folder) => {
        (out[parent(folder.path)] ||= []).push(folder);
        return out;
      }, {}),
    [folders],
  );
  ...
  const branch = (path: string, depth: number): React.ReactNode =>
    (tree[path] || []).map((folder) => {
      const children = tree[folder.path] || [];
      const isOpen =
        expanded.has(folder.path) ||
        active === folder.path ||
        active.startsWith(`${folder.path}/`);
      return (
        <React.Fragment key={folder.path}>
          <div
            className={`document-tree-row ${active === folder.path ? "active" : ""}`}
          ...
          >
            ...
            <button type="button" onClick={() => open(folder.path)}>
              <span>▰</span>
              {folder.name}
            </button>
          </div>
          {children.length > 0 && isOpen
            ? branch(folder.path, depth + 1)
            : null}
        </React.Fragment>
      );
    });
  if (isClosed)
    return (
      <aside className="document-tree collapsed">
        <button
          className="document-tree-reopen"
          type="button"
          onClick={() => setIsClosed(false)}
          aria-label="Open folder structure"
    ...
    <aside className="document-tree">
      <header>
        ...
        <div className="document-tree-head-actions">
          ...
          <button
            className="document-tree-close"
            type="button"
            onClick={() => setIsClosed(true)}
            aria-label="Close folder structure"
            title="Close folder structure"
          >
            ×
          </button>
        </div>
      </header>
      <div className={`document-tree-row root ${!active ? "active" : ""}`}>
        <button
          className="document-tree-toggle"
          type="button"
          onClick={() => toggle("")}
          aria-label="Toggle root folders"
        >
          {expanded.has("") ? "⌄" : "›"}
        </button>
      ...
      {expanded.has("") && branch("", 0)}
      {!folders.length && (
        <p className="document-tree-empty">No folders yet.</p>
      )}
    </aside>
  );
}

function DocumentGovernanceGuide() {
  return (
    <div className="document-guide">
      <div className="document-guide-intro">
        <span aria-hidden="true">✓</span>
        <div>
          <strong>Keep document data clean and audit-ready</strong>
          <p>
            Create one top-level folder for each change request, then place
            every artefact in the correct category.
          </p>
        </div>
      </div>
      <div className="document-guide-structure">
        <small>Recommended folder structure</small>
        <div className="document-guide-root">
          <span aria-hidden="true">▰</span>
          <div>
            <strong>CR-NUMBER</strong>
            <small>For example: CR-12345</small>
          </div>
        </div>
        <div className="document-guide-branches">
          <code>Execution Evidence</code>
          <code>Test Cases</code>
          <code>TCS Sign-off Certificates</code>
          <code>Etc.</code>
        </div>
      </div>
      <ul className="document-guide-rules">
        <li>Do not upload files directly at the repository root.</li>
        <li>Use clear file names containing a date or version.</li>
        <li>Avoid duplicate, temporary, and draft files.</li>
      </ul>
    </div>
  );
}

function DocumentFullscreenCredit() {
  return (
    <footer className="document-fullscreen-credit">
...
export default function DocumentPortal() {
  ...
  const [duplicate, setDuplicate] = useState<"keep" | "replace" | "cancel">(
    "keep",
  );
  ...
  useEffect(() => {
    setExpanded((current) => {
      ...
      for (let i = 1; i <= pieces.length; i += 1)
        next.add(pieces.slice(0, i).join("/"));
      return next;
    });
  }, [path]);
  ...
  const toggleTree = (nextPath: string) =>
    setExpanded((current) => {
      const next = new Set(current);
      next.has(nextPath) ? next.delete(nextPath) : next.add(nextPath);
      return next;
    });
  const closeDialog = () => {
  ...
  async function createFolder() {
    ...
    try {
      await api.post("/api/document-portal/folders", {
        path,
        name: folderName,
      });
      closeDialog();
  ...
  async function addDropped(event: DragEvent<HTMLDivElement>) {
    event.preventDefault();
    ...
    try {
      ...
      addFiles(
        values.map((value) => value.file),
        values.map((value) => value.path),
      );
    } catch {
      setError(
        new Error(
          "Some dropped files could not be read. Use the file or folder picker instead.",
        ),
      );
    }
  }
  ...
  async function rename() {
    ...
    setBusy("Renaming document…");
    try {
      await api.post("/api/document-portal/items/rename", {
  ...
  async function move() {
    ...
    setBusy("Moving document…");
    try {
      await api.post("/api/document-portal/items/move", {
        path: activeItem.path,
        destination,
      });
      closeDialog();
  ...
  async function download(url: string, filename: string) {
    ...
    try {
      ...
      setDownloadStatus(
        "Download started. Continue in your browser downloads if it does not open automatically.",
      );
    } catch (caught) {
      setError(caught);
      setDownloadStatus("");
  ...
  const visible = searchItems ?? data?.items ?? [];
  ...
  const selectAll = () =>
    setSelected(
      allSelected ? new Set() : new Set(visible.map((item) => item.path)),
    );
  const downloadOne = (item: Item) =>
    void download(
      item.is_folder
        ? `/api/document-portal/zip?path=${encodeURIComponent(item.path)}`
        : `/api/document-portal/download?path=${encodeURIComponent(item.path)}`,
      item.is_folder ? `${item.name}.zip` : item.name,
    );
  const downloadSelection = async () => {
    setDownloadStatus("Preparing selected items…");
    try {
      await api.downloadPost(
        "/api/document-portal/download-selection",
        { current_path: path, paths: [...selected] },
        selected.size === 1
          ? itemName([...selected][0])
          : `${itemName(path)}-selection.zip`,
      );
      setDownloadStatus(
        "Download started. Continue in your browser downloads if it does not open automatically.",
      );
  ...
  return (
    <div className="document-portal-page document-explorer-page">
      ...
      >
        <div className="document-mount">
          {isFullscreen && (
            <div className="document-fullscreen-brand" aria-label="QualityOps — Bank of Maharashtra">
              <img
                className="document-fullscreen-brand-logo"
                src="/qualityops-logo.png"
                alt=""
                aria-hidden="true"
              />
              <div>
                <b>
                  Quality<span>Ops</span>
                </b>
                <img
                  src="/bank-of-maharashtra-wordmark.png"
                  alt="Bank of Maharashtra"
                />
              </div>
            </div>
          )}
          <span className="document-library-icon">⌂</span>
          <strong>Mounted repository</strong>
          <code>{data?.repository_path || "Loading…"}</code>
          <button
            type="button"
            ...
            aria-label={
              isFullscreen
                ? "Exit full screen document library"
                : "Open document library full screen"
            }
            title={isFullscreen ? "Exit full screen (Esc)" : "Open full screen"}
          >
            {isFullscreen ? (
              <IconContract aria-hidden="true" />
            ) : (
          ...
          {!isSearching && (
            <>
              <label>
                ...
                <select
                  ...
                  onChange={(event) => {
                    ...
                    setSort(nextSort);
                    setOrder(nextOrder);
                    void load(path, nextSort, nextOrder);
                  }}
                >
              ...
              <button
                ...
                onClick={() =>
                  void download(
                    `/api/document-portal/zip?path=${encodeURIComponent(path)}`,
                    `${itemName(path)}.zip`,
                  )
                }
              >
        ...
        {busy && <p className="document-portal-status">{busy}</p>}
        {downloadStatus && (
          <div className="document-download-status">
            <strong>{downloadStatus}</strong>
            <span>
              Keep this page open while the browser receives the file.
            </span>
          </div>
        )}
        {!isSearching && data?.stats && (
          <div className="document-stats">
            <span>
              <strong>{data.stats.folders}</strong> folders
            </span>
            <span>
              <strong>{data.stats.files}</strong> files
            </span>
            <span>
        ...
        <div className="document-explorer-layout">
          <FolderTree
            ...
            expanded={expanded}
            toggle={toggleTree}
            open={(nextPath) => void openFolder(nextPath)}
          />
          <section className="document-explorer-main">
            <header className="document-contents-head">
              <div>
                <small>{isSearching ? "Search" : "Contents"}</small>
                <h2>
                  {isSearching
                    ? `Results for “${query}”`
                    : path
                ...
                <p>
                  {isSearching
                    ? `${visible.length}${searchTruncated ? "+" : ""} matching items`
                    : `${visible.filter((item) => item.is_folder).length} folders · ${visible.filter((item) => !item.is_folder).length} files`}
                </p>
              </div>
              <div className="document-contents-actions">
                <button
                  className="btn btn-small document-guide-button"
                  type="button"
                ...
                <tbody>
                  {visible.map((item) => (
                    <tr key={item.path}>
                      ...
                      <td>{item.is_folder ? "—" : bytes(item.size)}</td>
                      <td>{timestamp(item.modified_at)}</td>
                  ...
                  {!visible.length && (
                    <tr>
                      <td colSpan={6} className="document-empty">
                        {isSearching
                          ? "No documents or folders match this search."
        ...
        {isFullscreen && <DocumentFullscreenCredit />}
      </Card>
      {showFolderGuide && (
        <InfoModal
          title="Folder convention & data hygiene"
          onClose={() => setShowFolderGuide(false)}
        >
          <DocumentGovernanceGuide />
        </InfoModal>
      )}
      {dialog === "folder" && (
        <Modal title="New folder" onClose={closeDialog} variant="dialog">
      ...
      {dialog === "upload" && (
        ...
        >
          <div className="document-dialog">
            ...
            <div className="document-upload-source">
              <label className="btn">
                Choose files
                <input
                  ref={fileInput}
              ...
              <label>
                ...
                >
                  <option value="keep">Keep both (add number)</option>
                  <option value="replace">Replace existing</option>
                  <option value="cancel">Skip and report duplicate</option>
                </select>
              </label>
            </div>
            {uploads.length > 0 && (
              <>
                ...
                <div className="document-upload-list">
                  {uploads.map((upload) => (
                    <div
                      className={`document-upload-row ${upload.state}`}
                      key={upload.id}
                    >
                      <span>
                        {upload.state === "success"
                      ...
                      <div>
                        <strong>{upload.relativePath}</strong>
                        <small>
                          {upload.message ||
                            `${bytes(upload.file.size)} · Ready`}
                        </small>
                        <progress
                          value={
                            upload.file.size
                              ? Math.round(
                                  (upload.loaded / upload.file.size) * 100,
                                )
                              : 0
                          }
                          max="100"
                      ...
                      <button
                        ...
                        aria-label={`Remove ${upload.relativePath} from upload queue`}
                        title={
                          upload.state === "uploading"
                            ? "This file is currently uploading"
                            : upload.state === "success"
            ...
            {uploadSummary && (
              <div
                className={`document-upload-summary ${uploadSummary.failed ? "partial" : "success"}`}
              >
                <strong>
                ...
                <span>
                  {uploadSummary.success} uploaded · {uploadSummary.failed}{" "}
                  failed
                </span>
              </div>
            )}
      ...
      {confirmUpload && (
        <ConfirmModal
          title={`Upload ${uploads.filter((upload) => upload.state !== "success").length} ${uploads.filter((upload) => upload.state !== "success").length === 1 ? "file" : "files"}?`}
          message={
            <div className="document-upload-confirmation">
              <p>
                The selected files will be uploaded to <strong>{path || "Document Portal"}</strong>.
              </p>
              <dl>
                <div>
                  <dt>Files</dt>
                  <dd>{uploads.filter((upload) => upload.state !== "success").length}</dd>
                </div>
                <div>
                  <dt>Total size</dt>
                ...
                <div>
                  <dt>Duplicates</dt>
                  <dd>{duplicate === "keep" ? "Keep both" : duplicate === "replace" ? "Replace existing" : "Skip duplicate"}</dd>
                </div>
              </dl>
            </div>
          }
      ...
      {dialog === "move" && activeItem && (
        <Modal
          title={`Move “${activeItem.name}”`}
          onClose={closeDialog}
          variant="dialog"
        >
          <div className="document-dialog">
            <label>
              ...
              >
                <option value="">Document Portal (root)</option>
                {data?.folders
                  .filter(
                    (folder) =>
                      folder.path !== activeItem.path &&
                      !folder.path.startsWith(`${activeItem.path}/`),
                  )
                  .map((folder) => (
