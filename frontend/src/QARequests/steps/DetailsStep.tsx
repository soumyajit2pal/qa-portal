...
export function DetailsStep({ form, set }: Props) {
  ...
  useEffect(() => {
    api.get<ApplicationMasterOut[]>('/api/application-names').then(setApprovedNames).catch(() => {})
  }, [])
  ...
  useEffect(() => {
    ...
    const isApproved = approvedNames.some((a) => a.name === form.application_name)
        ...
        <div className="form-row">
          <Field label="Application Name *">
            ...
            >
              ...
              {approvedNames.map((a) => <option key={a.id} value={a.name}>{a.name}</option>)}
            ...
            {showOther && (
              <>
                <input
                  ...
                  onChange={(e) => set('application_name', e.target.value.toUpperCase())}
                  style={{ marginTop: 6 }}
                />
                <p className="muted small" style={{ margin: '4px 0 0' }}>
                  Automatically capitalised. A new name needs approval from an SM in your department
                  before it appears in this dropdown for everyone else -- your request isn't blocked
                  on that, it just shows as "Pending Approval" until then.
                </p>
              </>
            )}
          </Field>
          ...
          <Field label="Epic Number *"><input required value={form.epic_number} onChange={(e) => set('epic_number', e.target.value)} /></Field>
