...
// pattern/values as SAST_DAST_EDITABLE_STATUSES/AUTOMATION_EDITABLE_STATUSES/
...
// ---- Automation Testing lifecycle -- must mirror backend
// app/constants.py AUTOMATION_STATUSES. Auto-created from a QA Request when
// "Automation Testing" is selected.
export const AUTOMATION_STATUSES: string[] = [
  'DRAFT', 'SUBMITTED',
  'SM_APPROVAL_PENDING', 'RETURNED_BY_SM', 'SM_REJECTED',
  'DEPARTMENT_HEAD_APPROVAL_PENDING', 'RETURNED_BY_DEPARTMENT_HEAD', 'DEPARTMENT_HEAD_REJECTED',
...
export const AUTOMATION_TERMINAL_STATUSES: string[] = ['CLOSED', 'CANCELLED', 'SM_REJECTED', 'DEPARTMENT_HEAD_REJECTED']
export const AUTOMATION_EDITABLE_STATUSES: string[] = ['DRAFT', 'RETURNED_BY_SM', 'RETURNED_BY_DEPARTMENT_HEAD', 'RETURNED_BY_QA_LEAD']

// Automation Testing's own "Ready for Automation" readiness checklist --
// distinct from DEFAULT_CHECKLIST_ITEMS (Functional's) and
// DEFAULT_PERFORMANCE_CHECKLIST_ITEMS (Performance's). Must mirror backend
// app/constants.py DEFAULT_AUTOMATION_CHECKLIST_ITEMS. Only shown in the QA
// Request wizard while "Automation Testing" is one of the selected request
// types (see shell/QARequests.tsx::buildSteps) -- all items mandatory
// except "Manual testing completed...".
export interface AutomationChecklistItemDef {
  item: string
  owner: string
  is_mandatory: boolean
}
export const DEFAULT_AUTOMATION_CHECKLIST_ITEMS: AutomationChecklistItemDef[] = [
  { item: 'BRD / FRS / approved User Stories available', owner: 'Business / BA', is_mandatory: true },
  { item: 'Automation scope and objectives finalized', owner: 'Business / IT', is_mandatory: true },
  { item: 'Functional test scenarios reviewed and approved', owner: 'User Department', is_mandatory: true },
  { item: 'Manual testing completed and application functionality stabilized', owner: 'QA / Business', is_mandatory: false },
  { item: 'Stable application build deployed in the target environment', owner: 'Development Team', is_mandatory: true },
  { item: 'Dedicated and stable automation environment available', owner: 'IT / Development Team', is_mandatory: true },
  { item: 'Application URLs and required endpoints available', owner: 'Development Team', is_mandatory: true },
  { item: 'Test data requirements identified and reusable test data created', owner: 'User Department / Development Team', is_mandatory: true },
  { item: 'Test user accounts and role-based credentials available', owner: 'Business / IT', is_mandatory: true },
  { item: 'Stable UI element identifiers available, such as id, name, or data-testid', owner: 'Development Team', is_mandatory: true },
  { item: 'Application walkthrough provided to the Automation QA team', owner: 'User Department / Development Team', is_mandatory: true },
  { item: 'Expected results and business validation rules clearly documented', owner: 'Business / BA', is_mandatory: true },
  { item: 'Third-party services, OTP, CAPTCHA and payment dependencies identified with test-mode or bypass mechanisms', owner: 'Business / Development Team', is_mandatory: true },
  { item: 'Point of contact identified for functional queries, technical issues and environment support', owner: '', is_mandatory: true },
  { item: 'Out-of-scope and non-automatable scenarios documented and approved', owner: 'QA / Business', is_mandatory: true },
]
