...
export default function DepartmentPrompt() {
  ...
  async function save(e: React.FormEvent) {
    ...
    setSaveError(null)
    try {
      await api.patch('/api/auth/me', { department: selected })
      await refreshUser()
    } catch (err) {
      setSaveError(err)
    } finally {
        ...
        <section className="ldap-department-picker">
          ...
          <div className="ldap-department-options">{visibleDepartments.map((department) => {
            ...
            return <div className={isSelected ? 'selected' : ''} key={department.id}>
              <label><input type="radio" name="primary-department" checked={isSelected} onChange={() => setSelected(department.name)} disabled={busy} /><span>{department.name}</span></label>
              {isSelected && <strong>Primary</strong>}
            </div>
          })}</div>
        </section>
