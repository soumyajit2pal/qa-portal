...
export function Modal({ title, onClose, children, wide, variant = 'drawer', preventBackdropClose }: ModalProps) {
  ...
  if (variant === 'dialog') {
    return (
      <div className="modal-overlay modal-overlay-center" onClick={handleBackdropClick}>
        <div
          className={`dialog ${wide ? 'dialog-wide' : ''} ${shake ? 'modal-shake' : ''}`}
  ...
  return (
    <div className="modal-overlay" onClick={handleBackdropClick}>
      <div
        className={`drawer ${wide ? 'drawer-wide' : ''} ${shake ? 'modal-shake' : ''}`}
        onClick={(e) => e.stopPropagation()}
      >
        <div className="drawer-header">
          <h3>{title}</h3>
...
export function SignField({ userName, onSignedChange, disabled = false }: { userName?: string; onSignedChange: (name: string | null) => void; disabled?: boolean }) {
...
export function ApprovalDecisionButtons({
  ...
  extraControl, extraReady = true, signBlocked = false, signBlockedMessage,
  ...
  return (
    <>
      ...
      <button
        ...
        disabled={busy || !signedBy || !extraReady || signBlocked}
      ...
      {signBlocked && signBlockedMessage && (
        <span style={{ fontSize: 12.5, color: '#b45309' }}>{signBlockedMessage}</span>
      )}
    </>
  )
}
...
export function Table<T extends Record<string, any>>({ columns, rows, rowKey, onRowClick, pageSize = 5 }: TableProps<T>) {
  ...
  return (
    <div className="table-wrap">
      ...
      {filteredRows.length > 0 && (activeFilters.length > 0 || totalPages > 1) && (
        <div className="table-footer">
          ...
          {totalPages > 1 && (
            <div className="table-pagination">
              <span>{(currentPage - 1) * pageSize + 1}–{Math.min(currentPage * pageSize, filteredRows.length)} of {filteredRows.length}</span>
