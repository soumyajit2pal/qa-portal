...
export function useUserOptions(purpose: string, workspaceId?: number | null, defectId?: number, roles?: string, department?: string | null, repositoryGroup = false, testCaseId?: number) {
  const [users, setUsers] = useState<UserOption[]>([])
  const query = new URLSearchParams(repositoryGroup ? {} : { purpose })
