...
def department_head_decision(req_id: int, payload: schemas.PerformanceDeptHeadDecisionIn, db: Session = Depends(get_db),
    ...
    mirrors how SAST/DAST assign a Security Lead and Automation assigns an
    Engineer at their own equivalent step."""
    obj = _get_or_404(db, req_id)
    require_same_department(current_user, obj.department)
...
def readiness_decision(req_id: int, payload: schemas.ReadinessDecisionIn, db: Session = Depends(get_db),
    ...
    to be fully complete (every mandatory item ticked) before advancing to
    Feasibility -- unchanged from before. Failed is new: a discrepancy found
    during Readiness (beyond just an incomplete checklist -- e.g. something
    the assigned engineer notices is wrong with the request itself) returns
    it to the requester, with the QA Lead choosing (via
    require_dept_head_reapproval) whether the fix needs a fresh Department
    Head approval or can come straight back to Readiness once addressed (the
    default -- same assigned engineer, no re-approval needed)."""
    obj = _get_or_404(db, req_id)
...
# self-declare/QA-verify split as Functional's and Automation's checklists
