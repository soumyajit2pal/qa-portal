...
# disabled) is an Admin-only action, same restriction as SAST/Automation/
# Performance's own Edit Details (see update_sast/update_automation/
...
def update_functional(req_id: int, payload: schemas.FunctionalUpdate, db: Session = Depends(get_db),
                       current_user: models.User = Depends(get_current_user)):
    """Lets the requester (or QA) fix up a Functional Testing Request's
    details after it's been raised -- including the "Application & Change
    Details"/"Release & Environment" fields that are otherwise stuck once the
    QA Request gateway itself leaves DRAFT (see constants.
    GATEWAY_EDITABLE_STATUSES), since this request type has always shown
    them but, until now, had no way back in to correct one. Same
    requester-or-QA role gate and editable-status gate as the other modules'
    Edit Details endpoints. Application Name/Project Name/CR Number are
    further restricted to Admins only -- see _ADMIN_ONLY_FIELDS."""
...
def complete_qa(req_id: int, payload: schemas.CommentIn, db: Session = Depends(get_db),
    ...
    Blocked while any SAST/DAST/Automation/Performance request linked to the
    *same gateway QA Request* hasn't itself reached a terminal state -- this
    ...
    open_automation = [
        a.request_id for a in (gateway.linked_automation_requests if gateway else []) if a.status not in AUTOMATION_TERMINAL_STATUSES
    ]
    open_performance = [
    ...
    open_all = open_security + open_automation + open_performance
    if open_all:
        raise HTTPException(
            400,
