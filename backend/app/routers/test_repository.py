...
def export_test_repository(
    ...
    """Export a complete project or folder-subtree repository snapshot.

    UI filters deliberately do not limit the export: this is a governed
    snapshot of the selected Project/folder, including every status and
    every retained step and review event. A folder export includes the
    selected folder and every nested subfolder at any depth.
    """
    project = _get_project_or_404(db, project_id)
    require_project_visibility(db, project_id, current_user)
    all_folders = (db.query(models.TestFolder).filter_by(project_id=project_id)
                   .order_by(models.TestFolder.name).all())
    folder_by_id = {folder.id: folder for folder in all_folders}
    selected_folder = None
    scoped_folder_ids = None
    if folder_id is not None:
        selected_folder = folder_by_id.get(folder_id)
        if not selected_folder:
            raise HTTPException(404, "Folder not found in this project")
        scoped_folder_ids = ({folder_id} | _descendant_folder_ids(db, folder_id)) & set(folder_by_id)

    folders = all_folders if scoped_folder_ids is None else [
        folder for folder in all_folders if folder.id in scoped_folder_ids
    ]
    case_query = db.query(models.TestCase).filter_by(project_id=project_id)
    if scoped_folder_ids is not None:
        case_query = case_query.filter(models.TestCase.folder_id.in_(scoped_folder_ids))
    cases = case_query.order_by(models.TestCase.test_case_key).all()
    ...
    scope_path = folder_path(selected_folder.id) if selected_folder else None
    workbook = new_workbook()
    add_summary_sheet(
        workbook,
        f"{project.project_key} · {selected_folder.name if selected_folder else 'Test Repository'}",
        ("Complete folder-subtree definition and QA review snapshot."
         if selected_folder else "Complete repository definition and QA Lead review snapshot."),
        [
            ("Project", project.name), ("Project ID", project.project_key),
            ("Export scope", "Selected folder and nested subfolders" if selected_folder else "Entire project repository"),
            *([("Folder path", scope_path)] if selected_folder else []),
    ...
    if selected_folder:
        safe_folder_name = "".join(
            ...
            for character in selected_folder.name
        ).strip("_") or f"folder_{selected_folder.id}"
        filename = f"{project.project_key}_{safe_folder_name}_test_repository.xlsx"
    else:
...
def queue_test_repository_export(
    ...
    selected_folder = None
    if folder_id is not None:
        selected_folder = (db.query(models.TestFolder)
                           .filter_by(id=folder_id, project_id=project_id).first())
        if not selected_folder:
            raise HTTPException(404, "Folder not found in this project")
    if selected_folder:
        safe_folder_name = "".join(
